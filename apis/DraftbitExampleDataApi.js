...
export const usePeopleGET = (
  ...
  return useQuery(
    ['draftbitExampleDataPeopleGET', args],
    () => peopleGET(Constants, args, handlers, timeout),
    {
      ...
      staleTime,
      onSuccess: () =>
        queryClient.invalidateQueries(['draftbitExampleDataPeopleGETS']),
    }
  );
};
...
export const usersGET = async (Constants, { limit }, handlers, timeout) => {
  const paramsDict = {};
  if (limit !== undefined) {
...
export const useUsersGET = (
  ...
  return useQuery(
    ['Users', args],
    () => usersGET(Constants, args, handlers, timeout),
    {
      refetchInterval,
      refetchOnWindowFocus,
      refetchOnMount,
