...
const DeleteEventScreen = props => {
  const { theme, navigation } = props;
  const dimensions = useWindowDimensions();
  const Constants = GlobalVariables.useValues();
  const Variables = Constants;
  const shootrSupabaseDBAPIEventsListDeleteDELETE =
    ShootrSupabaseDBAPIApi.useEventsListDeleteDELETE();
        ...
        <Button
          ...
          onPress={() => {
            const handler = async () => {
              try {
                (
                  await shootrSupabaseDBAPIEventsListDeleteDELETE.mutateAsync({
...
export default withTheme(DeleteEventScreen);
